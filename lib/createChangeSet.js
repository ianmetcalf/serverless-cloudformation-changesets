...
const createChangeSet = (plugin, stackName, changeSetName, changeSetType) => {
  ...
  const params = {
    ...
    Parameters: getParameters(plugin),
...
function getParameters(plugin) {
  const compiledTemplate = plugin.serverless.service.provider.compiledCloudFormationTemplate || {}

  const overrides = (plugin.options.parameterOverrides || '').split(/\s+/).reduce((acc, part) => {
  ...
  const parameters = Object.keys(compiledTemplate.Parameters || {}).map(key => {
    if (key in overrides) {
      return {
        ParameterKey: key,
